...
public class ACMDockerJavaService extends AbstractDockerJavaExecutor {
    ...
    @Override
    public ExecRes execute0(TestCase testCase, String execId, ExecuteReq executeReq) {
...
//                    .withTty(Boolean.TRUE)
                    ...
                    .awaitCompletion(executeReq.getTimeout(), TimeUnit.MILLISECONDS);
    ...
    public String createExec(ExecuteReq executeReq, String containerId) {
        return dockerClient.execCreateCmd(containerId)
                .withWorkingDir(WORKING_DIR)
//                .withCmd("time", "-f", "%e %M %x", "java", "-Xmx " + executeReq.getMemory(), "Main")
    ...
    public String createAndStartContainer(ExecuteReq executeReq, String classDir) {
        String containerId = dockerClient.createContainerCmd("jre21")
                .withName("sandbox_java_" + executeReq.getId())
                .withHostConfig(
                        HostConfig.newHostConfig()
                                .withNetworkMode("none")
                                .withAutoRemove(true)
                                // 单位 Byte
                                .withMemory((long) executeReq.getMemory())
